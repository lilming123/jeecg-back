...
public class StorageController extends JeecgController<Storage, IStorageService>{
	...
	public Result<IPage<Storage>> queryPageList(Storage storage,
        ...
        if(hasQuery != null && "true".equals(hasQuery)){
            ...
            List<Storage> list = storageService.queryTreeListNoPage(queryWrapper);
            IPage<Storage> pageList = new Page<>(1, 10, list.size());
	 ...
	 public Result<List<SelectTreeModel>> loadTreeChildren(@RequestParam(name = "pid") String pid) {
		 ...
		 try {
			 List<SelectTreeModel> ls = storageService.queryListByPid(pid);
			 result.setResult(ls);
	 ...
	 public Result<List<SelectTreeModel>> loadTreeRoot(@RequestParam(name = "async") Boolean async, @RequestParam(name = "pcode") String pcode) {
		 Result<List<SelectTreeModel>> result = new Result<>();
		 try {
			 List<SelectTreeModel> ls = storageService.queryListByCode(pcode);
			 if (!async) {
				 loadAllChildren(ls);
			 }
			 result.setResult(ls);
	 ...
	 private void loadAllChildren(List<SelectTreeModel> ls) {
		 for (SelectTreeModel tsm : ls) {
			 List<SelectTreeModel> temp = storageService.queryListByPid(tsm.getKey());
      ...
      * @return
      */
	//@AutoLog(value = "仓库管理-获取子数据")
	@ApiOperation(value="仓库管理-获取子数据", notes="仓库管理-获取子数据")
	@GetMapping(value = "/childList")
	public Result<IPage<Storage>> queryPageList(Storage storage,HttpServletRequest req) {
		QueryWrapper<Storage> queryWrapper = QueryGenerator.initQueryWrapper(storage, req.getParameterMap());
    ...
    /**
      ...
      * @return
      */
	//@AutoLog(value = "仓库管理-批量获取子数据")
    @ApiOperation(value="仓库管理-批量获取子数据", notes="仓库管理-批量获取子数据")
	...
	public Result<String> add(@RequestBody Storage storage) {
	...
	public Result<String> edit(@RequestBody Storage storage) {
    ...
    //@RequiresPermissions("base:storage:deleteBatch")
	...
	public Result<Storage> queryById(@RequestParam(name="id",required=true) String id) {
    ...
    * @param storage
    */
    //@RequiresPermissions("base:storage:exportXls")
    @RequestMapping(value = "/exportXls")
    public ModelAndView exportXls(HttpServletRequest request, Storage storage) {
		return super.exportXls(request, storage, Storage.class, "仓库管理");
    }
    ...
    * @return
    */
    //@RequiresPermissions("base:storage:importExcel")
